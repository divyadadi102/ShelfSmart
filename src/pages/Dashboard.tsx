...
import { AreaChart, Area, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, BarChart, Bar, PieChart, Pie, Cell, LabelList } from 'recharts';
...
const Dashboard = () => {
  ...
  const [topTab, setTopTab] = useState<'week' | '2weeks' | 'month'>('week');
  const [bottomTab, setBottomTab] = useState<'week' | '2weeks' | 'month'>('week');
  ...
  useEffect(() => {
    ...
    if (topTab === 'week') start.setDate(end.getDate() - 6);
    else if (topTab === '2weeks') start.setDate(end.getDate() - 13);
    else if (topTab === 'month') start.setDate(end.getDate() - 29);
  ...
  function getRevenuePeriodParams(predictionPeriod: string, latestDate: string | null) {
    ...
    let today: Date;
    if (latestDate) {
      today = new Date(latestDate);
      today.setDate(today.getDate() + 1); // latestDate + 1
    } else {
      today = new Date('2017-08-16');
    }
...
if (bottomTab === 'week') start.setDate(end.getDate() - 6);
else if (bottomTab === '2weeks') start.setDate(end.getDate() - 13);
else if (bottomTab === 'month') start.setDate(end.getDate() - 29);
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <div className="flex items-center space-x-4">
            {businessName && (
              <h1 className="text-3xl font-bold text-blue-600">{businessName}</h1>
            )}
              ...
              <Card className="shadow-lg h-full">
                <CardHeader>
                  <CardTitle className="flex items-center space-x-2">
                    ...
                    <span>Product Sales Forecast</span>
                  </CardTitle>
                  <CardDescription>
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      <BarChart data={products}>
                        ...
                        <Bar dataKey="expected" fill="#3b82f6">
                          <LabelList dataKey="expected" position="top" formatter={(value: number) => value.toFixed(1)} />
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                  )}
            ...
            <CardHeader>
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <CardDescription>Most sold products for selected period</CardDescription>
                    </div>
                    <div className="flex space-x-2 mt-2">
                      ...
                      <Button
                        variant={topTab === '2weeks' ? 'default' : 'outline'}
                        onClick={() => setTopTab('2weeks')}
                      >
                        Last 2 Weeks
                      </Button>
                      <Button
            ...
            <CardHeader>
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <CardDescription>
                      Lowest sold products for selected period
                    </CardDescription>
                  </div>
                  <div className="flex space-x-2 mt-2">
                    <Button
                      variant={bottomTab === 'week' ? 'default' : 'outline'}
                      onClick={() => setBottomTab('week')}
                    >
                      Last Week
                    </Button>
                    <Button
                      variant={bottomTab === '2weeks' ? 'default' : 'outline'}
                      onClick={() => setBottomTab('2weeks')}
                    >
                      Last 2 Weeks
                    </Button>
                    <Button
                      variant={bottomTab === 'month' ? 'default' : 'outline'}
                      onClick={() => setBottomTab('month')}
                    >
                      Last Month
                    </Button>
                  </div>
